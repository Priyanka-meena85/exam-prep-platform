...
import sqlite3, json, os, time, random, re, subprocess, hashlib, shutil
...
def migrate_db(db):
    ...
    if cursor.fetchone()[0] == 0:
        ...
        db.execute("INSERT INTO users (id, username, password_hash) VALUES (1, 'admin', ?)", (admin_hash,))
        db.commit()

    # 2. Check and migrate mock_tests
    cursor.execute("PRAGMA table_info(mock_tests)")
    mock_test_columns = [col[1] for col in cursor.fetchall()]
    if 'user_id' not in mock_test_columns:
        db.execute("ALTER TABLE mock_tests ADD COLUMN user_id INTEGER REFERENCES users(id) DEFAULT 1")
    if 'name' not in mock_test_columns:
        db.execute("ALTER TABLE mock_tests ADD COLUMN name TEXT")
    if 'subject' not in mock_test_columns:
        db.execute("ALTER TABLE mock_tests ADD COLUMN subject TEXT")
    if 'topic_id' not in mock_test_columns:
        db.execute("ALTER TABLE mock_tests ADD COLUMN topic_id INTEGER REFERENCES topics(id)")
    db.commit()

    # 3. Check and migrate study_sessions
    cursor.execute("PRAGMA table_info(study_sessions)")
    study_session_columns = [col[1] for col in cursor.fetchall()]
    if 'user_id' not in study_session_columns:
        db.execute("ALTER TABLE study_sessions ADD COLUMN user_id INTEGER REFERENCES users(id) DEFAULT 1")
        db.commit()

    # 4. Check and migrate topic_mastery
    cursor.execute("PRAGMA table_info(topic_mastery)")
    topic_mastery_columns = [col[1] for col in cursor.fetchall()]
    if 'user_id' not in topic_mastery_columns:
        db.execute("ALTER TABLE topic_mastery RENAME TO topic_mastery_old")
        db.execute('''
        ...
        ''')
        db.execute('''
            INSERT INTO topic_mastery (id, user_id, topic_id, diagnostic_score, current_score, study_hours, status, last_studied, test_count)
            SELECT id, 1, topic_id, diagnostic_score, current_score, study_hours, status, last_studied, test_count FROM topic_mastery_old
        ''')
        db.execute("DROP TABLE topic_mastery_old")
        db.commit()

    # 5. Check and migrate settings
    cursor.execute("PRAGMA table_info(settings)")
    settings_columns = [col[1] for col in cursor.fetchall()]
    if 'user_id' not in settings_columns:
        db.execute("ALTER TABLE settings RENAME TO settings_old")
        db.execute('''
        ...
        ''')
        db.execute("DROP TABLE settings_old")
        db.commit()
...
@app.before_request
def require_login():
    allowed = ['login', 'register', 'static', None]
...
def index():
    ...
    # Overall stats (scoped to current user)
    tests = db.execute('''
    ...
    # Topic mastery summary (scoped to current user via LEFT JOIN)
    topics = db.execute('''
        SELECT tm.*, t.id as topic_id, t.name, t.subject, t.paper, t.weightage,
               COALESCE(tm.test_count, 0) as test_count
        FROM topics t
        LEFT JOIN topic_mastery tm ON tm.topic_id = t.id AND tm.user_id = ?
        ORDER BY tm.current_score ASC NULLS FIRST
    ''', (user_id,)).fetchall()
 
    weak_topics = [t for t in topics if t['current_score'] is not None and t['current_score'] < 60 and t['test_count'] > 0]
    strong_topics = [t for t in topics if t['current_score'] is not None and t['current_score'] >= 70]
    untested = [t for t in topics if t['test_count'] == 0]
 
    # Recent errors (scoped to current user)
    recent_errors = db.execute('''
        ...
        JOIN mock_tests mt ON el.test_id = mt.id
        WHERE mt.user_id = ? AND el.resolved = 0 
        ORDER BY el.created_at DESC LIMIT 10
    ''', (user_id,)).fetchall()
 
    # Weekly progress (scoped to current user)
    week_ago = (datetime.now() - timedelta(days=7)).strftime('%Y-%m-%d')
    weekly_tests = db.execute('''
        SELECT date(completed_at) as d, AVG(score) as avg 
        FROM mock_tests 
        WHERE user_id = ? AND status="completed" AND completed_at >= ? 
        GROUP BY d ORDER BY d
    ''', (user_id, week_ago)).fetchall()
 
    # Accuracy by error type (scoped to current user)
    ...
    # Paper-wise performance (scoped to current user)
    paper_perf = db.execute('''
        SELECT paper, COUNT(*) as tests, AVG(score) as avg 
        FROM mock_tests 
        WHERE user_id = ? AND status="completed" 
        GROUP BY paper
    ''', (user_id,)).fetchall()
 
    settings = dict(db.execute('SELECT key, value FROM settings WHERE user_id = ?', (user_id,)).fetchall())
 
    # Today's attempted tests (scoped to current user)
    today_str = datetime.now().strftime('%Y-%m-%d')
    today_tests = db.execute('''
        SELECT mt.*, 
               (SELECT COUNT(*) FROM test_responses WHERE test_id = mt.id) as attempted_questions,
               t.name as topic_name
        FROM mock_tests mt
        LEFT JOIN topics t ON mt.topic_id = t.id
        WHERE mt.user_id = ? AND date(mt.started_at) = ?
        ORDER BY mt.started_at DESC
    ''', (user_id, today_str)).fetchall()

    today_tests_processed = []
    for row in today_tests:
        r = dict(row)
        try:
            dt = datetime.fromisoformat(r['started_at'])
            r['attempt_time_formatted'] = dt.strftime('%I:%M %p')
            r['attempt_date_formatted'] = dt.strftime('%Y-%m-%d')
        except Exception:
            r['attempt_time_formatted'] = r['started_at']
            r['attempt_date_formatted'] = r['started_at'][:10] if r['started_at'] else ''
        today_tests_processed.append(r)
    ...
    return render_template('index.html',
        ...
        today_tests=today_tests_processed)
...
def test_setup():
    ...
    # Get subjects and topics for rendering the setup form
    topics = db.execute('SELECT * FROM topics ORDER BY paper, subject, name').fetchall()
    subjects = sorted(list(set(t['subject'] for t in topics)))
    
    if request.method == 'POST':
        paper = request.form.get('paper', 'II')
        num_qs = int(request.form.get('num_questions', 50))
        selected_subject = request.form.get('subject', 'all')
        selected_topic_str = request.form.get('topic', 'all')
        difficulty = request.form.get('difficulty', 'all')
        focus_weak = request.form.get('focus_weak') == 'on'

        # Fetch already attempted question IDs for this user
        attempted_rows = db.execute('''
            SELECT DISTINCT tr.question_id
            FROM test_responses tr
            JOIN mock_tests mt ON tr.test_id = mt.id
            WHERE mt.user_id = ?
        ''', (session['user_id'],)).fetchall()
        attempted_ids = [row[0] for row in attempted_rows]

        # Build query for candidate questions
        query = '''
            SELECT q.*, t.name as topic_name, t.subject as topic_subject
            FROM questions q
        ...
        # Filter by selected topic (takes precedence over subject because a topic is specific)
        if selected_topic_str != 'all':
            query += ' AND t.id = ?'
            params.append(int(selected_topic_str))
        ...
        # Filter by focus on weak topics
        if focus_weak:
            weak_topic_ids = [str(t['topic_id']) for t in db.execute(
                'SELECT topic_id FROM topic_mastery WHERE user_id = ? AND (current_score < 60 OR current_score IS NULL)',
                (session['user_id'],)
            ).fetchall()]
            if weak_topic_ids:
        ...
        if selected_topic_str != 'all':
            topic_row = db.execute('SELECT name, subject FROM topics WHERE id = ?', (int(selected_topic_str),)).fetchone()
            if topic_row:
                test_name = f"Topic Test: {topic_row['name']}"
                topic_id_val = int(selected_topic_str)
                subj_val = topic_row['subject']
        elif selected_subject != 'all':
            test_name = f"Subject Test: {selected_subject}"
            subj_val = selected_subject
        else:
            if paper != 'both':
                test_name = f"Paper {paper} Test"
            else:
                test_name = "Mixed Mock Test"

        # Create mock_test row in database
        cursor = db.cursor()
        cursor.execute(
            '''INSERT INTO mock_tests (user_id, name, subject, topic_id, started_at, paper, total_questions, max_score, status)
               VALUES (?,?,?,?,?,?,?,?,"in_progress")''',
            (session['user_id'], test_name, subj_val, topic_id_val, datetime.now().isoformat(), paper, len(selected), len(selected))
        )
    ...
    return render_template('test_setup.html', topics=topics, subjects=subjects)
...
def submit_answer():
    ...
    # Store attempt immediately in database on-the-fly
    db = get_db()
    test_id = session.get('test_id')
    if test_id:
        existing = db.execute('SELECT id FROM test_responses WHERE test_id = ? AND question_id = ?', 
                              (test_id, q['id'])).fetchone()
        if existing:
            db.execute('''
                UPDATE test_responses 
                SET selected_option = ?, is_correct = ?, time_spent_sec = ?
                WHERE id = ?
            ''', (selected, 1 if is_correct else 0, time_spent, existing['id']))
        else:
            db.execute('''
                INSERT INTO test_responses (test_id, question_id, selected_option, is_correct, time_spent_sec)
                VALUES (?, ?, ?, ?, ?)
            ''', (test_id, q['id'], selected, 1 if is_correct else 0, time_spent))
        db.commit()
...
def finish_test():
    ...
    db.execute('UPDATE mock_tests SET completed_at=?, score=?, time_taken_sec=?, status="completed" WHERE id=?',
        (datetime.now().isoformat(), score, total_time, test_id))

    # Clear pre-existing on-the-fly responses to prevent duplicate entries
    db.execute('DELETE FROM test_responses WHERE test_id = ?', (test_id,))
    ...
    for tid, scores in topic_scores.items():
        ...
        existing = db.execute('SELECT * FROM topic_mastery WHERE user_id=? AND topic_id=?', (session['user_id'], tid)).fetchone()
        if existing:
            new_score = round((existing['current_score'] or 0) * 0.6 + pct * 0.4, 1) if existing['current_score'] else pct
            db.execute('UPDATE topic_mastery SET current_score=?, test_count=test_count+1, last_studied=?, status=? WHERE user_id=? AND topic_id=?',
                (new_score, datetime.now().isoformat(), 'in_progress' if new_score < 70 else 'stable', session['user_id'], tid))
        else:
            db.execute('INSERT INTO topic_mastery (user_id, topic_id, current_score, test_count, status, last_studied) VALUES (?,?,?,1,?,?)',
                (session['user_id'], tid, pct, 'in_progress' if pct < 70 else 'stable', datetime.now().isoformat()))
    ...
    session['last_result'] = {
        ...
        'time_taken': total_time, 'paper': 'I' if len(questions) > 0 and questions[0].get('topic_name','').startswith('Raj') else 'II'
    }
...
def results(test_id):
    ...
    db = get_db()
    test = db.execute('SELECT * FROM mock_tests WHERE id=? AND user_id=?', (test_id, session['user_id'])).fetchone()
...
def analytics():
    ...
    # Progress over time
    progress = [dict(r) for r in db.execute('SELECT id, date(completed_at) as d, paper, score, time_taken_sec FROM mock_tests WHERE user_id=? AND status="completed" ORDER BY completed_at', (user_id,)).fetchall()]
    ...
    paper1 = [dict(r) for r in db.execute('SELECT id, score, time_taken_sec, date(completed_at) as d FROM mock_tests WHERE user_id=? AND paper="I" AND status="completed" ORDER BY completed_at DESC LIMIT 5', (user_id,)).fetchall()]
    paper2 = [dict(r) for r in db.execute('SELECT id, score, time_taken_sec, date(completed_at) as d FROM mock_tests WHERE user_id=? AND paper="II" AND status="completed" ORDER BY completed_at DESC LIMIT 5', (user_id,)).fetchall()]
    ...
    error_trends = [dict(r) for r in db.execute('''
        ...
        JOIN mock_tests mt ON el.test_id = mt.id
        WHERE mt.user_id = ?
        GROUP BY d, el.error_type ORDER BY d
    ''', (user_id,)).fetchall()]
    ...
    time_trend = [dict(r) for r in db.execute('''
        ...
        WHERE mt.user_id = ? AND mt.status="completed" GROUP BY mt.id ORDER BY mt.completed_at
    ''', (user_id,)).fetchall()]
    ...
    for i in range(30, -1, -1):
        ...
        day_data = db.execute('SELECT COUNT(*) as tests, AVG(score) as avg FROM mock_tests WHERE user_id=? AND date(completed_at)=? AND status="completed"', (user_id, d)).fetchone()
...
def error_log_view():
    ...
    query = 'SELECT el.*, t.name as topic_name, q.question_text FROM error_log el JOIN topics t ON el.topic_id = t.id JOIN questions q ON el.question_id = q.id JOIN mock_tests mt ON el.test_id = mt.id WHERE mt.user_id = ?'
...
def resolve_error():
    ...
    # Verify ownership
    valid = db.execute('SELECT el.id FROM error_log el JOIN mock_tests mt ON el.test_id = mt.id WHERE el.id = ? AND mt.user_id = ?', (data['error_id'], session['user_id'])).fetchone()
    if not valid:
...
def update_settings():
    ...
    for k, v in data.items():
        db.execute('INSERT OR REPLACE INTO settings (user_id, key, value) VALUES (?,?,?)', (session['user_id'], k, str(v)))
    db.commit()
    return jsonify({'status': 'ok'})
...
def log_study_session():
    ...
    data = request.json
    db = get_db()
    db.execute('INSERT INTO study_sessions (user_id, date, topic_id, duration_min, mcqs_solved, score, notes) VALUES (?,?,?,?,?,?,?)',
        (session['user_id'], datetime.now().isoformat(), data['topic_id'], data['duration'], data['mcqs'], data.get('score', 0), data.get('notes', '')))
    
    # Update topic mastery scoped to user
    existing = db.execute('SELECT * FROM topic_mastery WHERE user_id=? AND topic_id=?', (session['user_id'], data['topic_id'])).fetchone()
    if existing:
        db.execute('UPDATE topic_mastery SET study_hours = study_hours + ? WHERE user_id=? AND topic_id = ?', (data['duration']/60.0, session['user_id'], data['topic_id']))
    else:
        db.execute('INSERT INTO topic_mastery (user_id, topic_id, study_hours, status) VALUES (?,?,?,?)', (session['user_id'], data['topic_id'], data['duration']/60.0, 'in_progress'))
...
GEMINI_CLI = shutil.which('gemini') or os.path.expanduser('~/.nvm/versions/node/v20.20.2/bin/gemini')
...
def _call_gemini_fallback(prompt):
    import re
    # We can analyze the prompt to check if it's a deep-dive or chat
    if "Concept Explanation" in prompt or "deep-dive" in prompt.lower() or "QUESTION:" in prompt:
        topic_m = re.search(r'TOPIC:\s*(.*)', prompt)
        subject_m = re.search(r'SUBJECT:\s*(.*)', prompt)
        correct_m = re.search(r'CORRECT ANSWER:\s*(.*)', prompt)
        explanation_m = re.search(r'OFFICIAL EXPLANATION:\s*(.*)', prompt, re.DOTALL)
        
        topic = topic_m.group(1).strip() if topic_m else "General Computer Anudeshak Syllabus"
        subject = subject_m.group(1).strip() if subject_m else "Computer Science"
        correct_opt = correct_m.group(1).strip() if correct_m else "A"
        
        explanation = ""
        if explanation_m:
            exp_text = explanation_m.group(1).strip()
            if "RELEVANT" in exp_text:
                exp_text = exp_text.split("RELEVANT")[0].strip()
            explanation = exp_text
            
        if not explanation or explanation.lower() == "none provided":
            explanation = "This topic contains critical conceptual elements frequently tested in RPSC and RSSB examinations. Please review standard textbook definitions and practice multiple-choice questions regularly."

        explanation = explanation.replace('\\n', '\n')

        response = """
1. Concept Explanation (Overview)
The concept of **{topic}** (under the subject **{subject}**) is a core area in the Computer Anudeshak syllabus. A clear understanding of this topic enables candidates to solve both direct and application-based questions efficiently.

The correct option is **{correct_opt}**. According to the database explanation:
{explanation}

Understanding this area also helps in developing problem-solving skills for other related subjects. Regular practice of these questions under timed conditions is highly recommended to improve overall speed and accuracy.

2. Key Facts (Must-Remember)
*   Fact 1: {topic} is highly relevant for the Computer Anudeshak Exam.
*   Fact 2: Standard references (like RPSC/RSSB previous question papers) show that direct definitions are frequently asked.
*   Fact 3: A solid theoretical understanding prevents memory lapses under exam pressure.
*   Fact 4: The correct option is verified to match the standard syllabus guidelines.

3. Exam Tips (Advice)
*   Tip 1: Watch out for subtle phrasing differences in the multiple-choice options.
*   Tip 2: Always read the entire question (both English and Hindi translations if available) before marking your response.
*   Tip 3: Practice under timed conditions to simulate the actual test environment.
""".format(topic=topic, subject=subject, correct_opt=correct_opt, explanation=explanation)
        if subject == 'History':
            response += """
4. Storyline (Mnemonic)
To remember this historical fact about **{topic}**, associate it with the context of standard chronological events. A quick memory trick is to write a short 2-sentence summary of the event immediately after studying it.
""".format(topic=topic)
        return response.strip()
    else:
        # It's a follow-up chat
        user_q_m = re.search(r"User's new question:\s*(.*)", prompt, re.DOTALL)
        user_q = user_q_m.group(1).strip() if user_q_m else "Can you explain this further?"
        
        topic_m = re.search(r'TOPIC:\s*(.*)', prompt)
        topic = topic_m.group(1).strip() if topic_m else "General Topic"
        
        response = """
### Tutor Explanation on **{topic}**

In response to your query: *"{user_q}"*

Here is a clear, exam-focused explanation:

1. **Core Concept**: When preparing for the Computer Anudeshak exam, it is vital to focus on standard conceptual definitions and their implementation details.
2. **Key Context**: In Rajasthan-specific competitive exams, RPSC/RSSB questions tend to emphasize standard syntax, primary functionalities, and historical milestones. Make sure to consult standard board books.
3. **Study Strategy**: Keep tracking your weak topics in the Diagnostic Dashboard. Focus on error logs and retrying previously missed questions.
""".format(topic=topic, user_q=user_q)
        return response.strip()

def call_gemini(prompt, timeout=60):
    """Call Gemini CLI and return response text.
    Uses polling instead of timeout-based waits to avoid proot signal issues."""
    import threading
    
    # Fallback if GEMINI_CLI path doesn't exist or isn't on the system
    if not GEMINI_CLI or (not os.path.exists(GEMINI_CLI) and shutil.which('gemini') is None):
    ...
    try:
        ...
        output: list[str | None] = [None, None]
        done = [False]
        ...
        return response or _call_gemini_fallback(prompt)
    except Exception as e:
        return _call_gemini_fallback(prompt)
